...
def _make_compile_hook(model: torch.nn.Module, compile_options: Optional[dict] = None):
    """
    Create a one-time compilation hook that triggers when an internal counter reaches 2.
    
    This hook will compile the model using torch.compile when the internal counter reaches 2.
    The hook removes itself after execution.
    
    Args:
        model: The model to compile
        compile_options: Options to pass to torch.compile, if any
        
    Returns:
        A tuple of (compile_hook_function, hook_handle_ref)
    """
    remove_myself, hook_handle_ref = _create_one_time_hook_remover()
    
    # Using a list to hold the counter so it can be mutated in the closure
    counter = [0]
   
    @torch._dynamo.disable
    def compile_hook(module: torch.nn.Module, inputs):
        counter[0] += 1
        if counter[0] == 2:
            remove_myself()
            torch._dynamo.config.cache_size_limit = 32
            if compile_options:
                torch.nn.Module.compile(model, **compile_options)
            else:
                torch.nn.Module.compile(model)
            
    return compile_hook, hook_handle_ref
...
def setup_offload(
    ...
    compile_hook_key = str(model) + "compile_hook_registered"
    if enable_torch_compile and not hasattr(setup_offload, compile_hook_key):
        compile_hook, compile_hook_handle_ref = _make_compile_hook(model, compile_options)
        compile_hook_handle = model.register_forward_pre_hook(compile_hook)
        compile_hook_handle_ref.set_handle(compile_hook_handle)
        setup_offload.__setattr__(str(model) + compile_hook_key, True)
...
def auto_setup_offload(
    ...
    dist.barrier()
    configs = [configs]
    dist.broadcast_object_list(configs, src=MASTER_RANK, device=torch.device("cpu"))
    configs = configs[0]
    return [
        setup_offload(model, onload_device, offload_device, offload_param_names, numels, rank, world_size, enable_torch_compile, compile_options, offload_quantization_config)
