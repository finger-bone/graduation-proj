...
def generate_strategy(
    ...
    if strategy_generator_name == "greedy":
        result = greedy_strategy_generator(onload_time, compute_time, leaf_modules, params.get("greedy", {}))
        logger.info("Greedy strategy generation completed")
        return result
    if strategy_generator_name == "genetic":
        result = genetic_strategy_generator(onload_time, compute_time, leaf_modules, params.get("genetic", {}))
        logger.info("Genetic strategy generation completed")
        return result
    if strategy_generator_name == "gradient":
        result = gradient_strategy_generator(onload_time, compute_time, leaf_modules, params.get("gradient", {}))
